...
import com.hodo.iiot.group2.hdcloud.bank.account.cambodia.feign.RemoteReportService;
...
public class HdReportSercieImpl implements HdReportService {
    ...
    public List<BankDetailStatistics> getBankDetailList(String startTime,
        ...
        //查询账单明细
        List<BankDetailStatistics> bankDetailList = remoteReportService.getBankDetailList(startTime,endTime,
                accountId,TenantContextHolder.getTenantId());
        List<BankDetailStatistics> result = new ArrayList<>();
        //加入期初
        String balance = remoteReportService.getBankReportBalance(startTime,accountId,TenantContextHolder.getTenantId());
        //封装期末
    ...
    public List<ComDetailStatistics> getComDetailList(
        ...
        //查询账单明细
        List<ComDetailStatistics> comDetailList = remoteReportService.getComDetailList(startTime,endTime,
                companyId,TenantContextHolder.getTenantId());
        List<ComDetailStatistics> result = new ArrayList<>();
        //加入期初
        String balance = remoteReportService.getComReportBalance(startTime,companyId,TenantContextHolder.getTenantId());
        BigDecimal beginBalance = BigDecimal.ZERO;
        if(StringUtil.isNotEmpty(balance)){
            beginBalance = new BigDecimal(balance);
        }
    ...
    public List<BankComStatistics> getBankComStatistics(String dateStart, String dateEnd){
        List<BankComStatistics> bankComStatisticsList = remoteReportService.getBankComStatistics(dateStart,dateEnd,TenantContextHolder.getTenantId());
    ...
    public List<ComBankStatistics> getComBankStatistics(String dateStart, String dateEnd){
        List<ComBankStatistics> comBankStatisticsList = remoteReportService.getComBankStatistics(dateStart,dateEnd,TenantContextHolder.getTenantId());
        BigDecimal sumIncome = BigDecimal.ZERO;
        BigDecimal sumPay = BigDecimal.ZERO;
        BigDecimal sumBegin = BigDecimal.ZERO;
        BigDecimal sumEnd = BigDecimal.ZERO;
    ...
    public List<BankStatistics> getBankStatistics(String dateStart, String dateEnd, String userId,
                                                  Integer tenantId){
        List<BankStatistics> bankStatisticsList = remoteReportService.getBankStatisticsList(dateStart,dateEnd,userId,tenantId);
