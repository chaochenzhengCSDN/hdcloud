package com.hodo.iiot.group2.data.hdcloud.bank.account.bill.controller.ty;
...
import com.hodo.iiot.group2.data.hdcloud.bank.account.bill.service.impl.ty.HdBankStatementTyServiceImpl;
...
@RestController
@AllArgsConstructor
@RequestMapping("/hdbankstatementty/ty")
@Api(value = "hdbankstatement", tags = "bank_account管理")
public class HdBankStatementTyController {

    private final HdBankStatementTyServiceImpl hdBankStatementTyService;
    ...
    public R getHdBankStatementPage(@RequestParam Map<String, Object> map) {
        ...
        Object ob = hdBankStatementTyService.page(page, queryWrapper);
        return R.ok(ob);
    }
    ...
    public R getHdBankStatementAll(@RequestParam Map<String, Object> map) {
        ...
        return R.ok(hdBankStatementTyService.list(queryWrapper));
    }
    ...
    public List<String> getSheetIds() {
        ...
        return hdBankStatementTyService.getSheetIds(tenantId);
    }
    ...
    public List<String> getRIds() {
        ...
        return hdBankStatementTyService.getRIds(tenantId);
    }
    ...
    public R getById(@PathVariable("id") String id) {
        DynamicDataSourceContextHolder.setDataSourceType(DataSourceId.BANK_ACCOUNT_TY);
        return R.ok(hdBankStatementTyService.getById(id));
    }
    ...
    public HdBankStatement selectOne(@RequestParam Map<String, Object> map) {
        ...
        jjUtil.handleAllParamsToWrapper(map, queryWrapper);
        try {
            return hdBankStatementTyService.getOne(queryWrapper);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
...
//
//        return R.ok(hdBankStatementTyService.save(hdBankStatementTy));
//    }
    ...
    @PutMapping
    @PreAuthorize("@pms.hasPermission('hdbankstatementty_edit_day')")
    public R updateById(@RequestBody HdBankStatement hdBankStatement) {
        DynamicDataSourceContextHolder.setDataSourceType(DataSourceId.BANK_ACCOUNT_TY);
        boolean b = hdBankStatementTyService.updateById(hdBankStatement);
        if (b) {
            return R.ok(b);
        } else {
            return R.failed(b);
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("@pms.hasPermission('hdbankstatementty_del')")
    public R removeById(@PathVariable String id) {
        DynamicDataSourceContextHolder.setDataSourceType(DataSourceId.BANK_ACCOUNT_TY);
        return R.ok(hdBankStatementTyService.removeById(id));
    }

    @ApiOperation(value = "通过ids删除", notes = "通过ids删除")
    @SysLog("通过ids删除")
    @PostMapping("/removeByIds")
    @PreAuthorize("@pms.hasPermission('hdbankstatementty_batch_del')")
    public R removeByIds(@RequestBody List<String> list) {
        DynamicDataSourceContextHolder.setDataSourceType(DataSourceId.BANK_ACCOUNT_TY);
        try {
            return R.ok(hdBankStatementTyService.removeByIds(list));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @PreAuthorize("@pms.hasPermission('hdbankstatementty_delete')")
    public R removeById(@RequestBody Map<String, Object> map) {
        ...
        return R.ok(hdBankStatementTyService.remove(wrapper));
    }
    ...
    public List<HdBankStatement> getBankStatementListBySql(@RequestParam Map<String, Object> map) {
        ...
        List list = hdBankStatementTyService.getBankStatementListBySql(moneyStart, moneyEnd, startTime, endTime,
                String.valueOf((Integer.parseInt(page) - 1) * Integer.parseInt(limit)), limit, map, tenantId, accountTypeList,
    ...
    public List<HdBankStatement> getAllBankAccountBySql(@RequestParam  Map<String, Object> params) {
        ...
        return hdBankStatementTyService.getAllBankAccountBySql(moneyStart, moneyEnd, startTime, endTime, params,
    ...
    public Map<String, Object> getOtherVal(@RequestParam  Map<String, Object> params) {
        ...
        Map<String, Object> map = hdBankStatementTyService.getOtherVal(moneyStart, moneyEnd, startTime, endTime, params,
                tenantId, accountId, accountTypeList, synStartTime, synEndTime);
        return map;
    }
    ...
    public synchronized BigDecimal selectMaxNo(@RequestParam(required = false) String accountDate) {
        ...
        Integer tenantId = TenantContextHolder.getTenantId();
        if (accountDate == null) {
            return hdBankStatementTyService.selectMaxNo(tenantId);
        } else {
            return hdBankStatementTyService.selectMaxNoByDate(accountDate, tenantId);
        }
    }
    ...
    @PreAuthorize("@pms.hasPermission('hdbankstatementty_import')")
    public synchronized R addList(@RequestBody List<HdBankStatement> list) {
        ...
        for (HdBankStatement hdBankStatementTy :
                list) {
//           BigDecimal bd =  hdBankStatementTyService.selectMaxNo(tenantId);
...
//            hdBankStatementTy.setNo(bd);
        ...
        boolean b = hdBankStatementTyService.saveBatch(list);
        if (b) {
            return R.ok("true");
        } else {
    ...
    public boolean deleteBatch(List<String> list) {
        DynamicDataSourceContextHolder.setDataSourceType(DataSourceId.BANK_ACCOUNT_TY);
        return hdBankStatementTyService.removeByIds(list);
    }
    ...
    public String getBankReportBalance(@RequestParam String startTime, @RequestParam String tenantId,
        ...
        DynamicDataSourceContextHolder.setDataSourceType(DataSourceId.BANK_ACCOUNT_TY);
        return hdBankStatementTyService.getBankReportBalance(startTime, tenantId, bankAccountId);
    }
    ...
    public String getComReportBalance(@RequestParam String startTime, @RequestParam String tenantId,
        ...
        DynamicDataSourceContextHolder.setDataSourceType(DataSourceId.BANK_ACCOUNT_TY);
        return hdBankStatementTyService.getComReportBalance(startTime, tenantId, companyId);
    }
